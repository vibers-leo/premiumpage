...
// Define the order of company pages
const COMPANY_PAGES = [
    'introduction',
    'global_network',
    'company_view'
...
export const CompanyNavigator = ({ currentSlug, lang }: CompanyNavigatorProps) => {
    // Check if the current slug is part of the company intro sequence
    const currentIndex = COMPANY_PAGES.indexOf(currentSlug);

    // If it's not in the list (e.g., greeting, history), don't show this specific navigator
    if (currentIndex === -1) return null;

    const totalPages = COMPANY_PAGES.length;
    const prevSlug = currentIndex > 0 ? COMPANY_PAGES[currentIndex - 1] : null;
    const nextSlug = currentIndex < totalPages - 1 ? COMPANY_PAGES[currentIndex + 1] : null;

    const prevLink = prevSlug ? `/${lang}/company/${prevSlug}` : "#";
    const nextLink = nextSlug ? `/${lang}/company/${nextSlug}` : "#";
