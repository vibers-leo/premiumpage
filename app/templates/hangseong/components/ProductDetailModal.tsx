                ...
                <div className="flex justify-between items-center p-6 border-b dark:border-neutral-800 border-neutral-200 dark:bg-neutral-900/95 bg-white/95 backdrop-blur z-20 sticky top-0">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm md:text-base text-blue-400 font-medium">{product.subtitle}</p>
                        </div>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 md:p-10 relative scrollbar-thin scrollbar-thumb-neutral-700 scrollbar-track-transparent">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-16">
                                ...
                                <div className="grid grid-cols-4 sm:grid-cols-5 gap-3">
                                    ...
                                    {[product.image, ...product.gallery.filter((img: string) => img !== product.image)].map((img: string, idx: number) => (
                                        <div
                                            ...
                                            className={`aspect-square relative dark:bg-white/5 bg-neutral-50 rounded-xl border overflow-hidden cursor-pointer transition-all group ${activeImage === img ? 'border-blue-500 ring-2 ring-blue-500/20' : 'dark:border-white/5 border-neutral-200 dark:hover:border-white/20 hover:border-neutral-300'}`}
                                        >
                                            <Image
                                                src={img}
                                                fill
                        ...
                        <div className="lg:col-span-5 space-y-8">
                            <div>
                                <h3 className="text-sm font-bold text-blue-500 uppercase tracking-widest mb-3">Product Overview</h3>
                            ...
                            <div className="dark:bg-neutral-800/30 bg-neutral-50 rounded-2xl p-6 border dark:border-white/5 border-neutral-200">
                                <h4 className="dark:text-white text-neutral-900 font-semibold mb-6 flex items-center gap-2 pb-4 border-b dark:border-white/5 border-neutral-200">
                                    <FileText size={18} className="text-blue-500" /> Technical Highlights
                                </h4>
                                <div className="space-y-4">
                                    {product.specs?.map((s: any, i: number) => (
                                        <div key={i} className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 group">
                                            ...
                                            <span className="dark:text-neutral-200 text-neutral-800 font-mono text-base dark:group-hover:text-blue-300 group-hover:text-blue-600 transition-colors">{s.value}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <button
                                onClick={() => {
                                    if (product.datasheet) {
                                        ...
                                        // Show alert if no datasheet available
                                        alert('Datasheet not available for this product. Please contact us for more information.')
                                    }
                                }}
                                className={`w-full py-4 ${product.datasheet ? 'bg-blue-600 hover:bg-blue-500' : 'bg-neutral-700 hover:bg-neutral-600'} text-white font-bold rounded-xl shadow-lg shadow-blue-900/20 transition-all flex items-center justify-center gap-3 group`}
                            >
                                <Download className="w-5 h-5 group-hover:-translate-y-1 transition-transform" />
                                {product.datasheet ? 'Download Datasheet' : 'Datasheet Not Available'}
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="border-t dark:border-white/5 border-neutral-200 pt-16 animation-fade-in">
                        <div className="mb-10 flex items-center gap-4">
                            ...
                            <h3 className="text-2xl md:text-3xl font-bold dark:text-white text-neutral-900">Detailed Specifications</h3>
                        </div>
